...
int main(int argc,char** argv) {
    ...
    for(int cnt = 0; cnt < 5; ++cnt) {
        ...
        std::cout << "发送有效的长度: " << strlen(message) << std::endl;
        unsigned long strLength = htonl(strlen(message));
        // 将数据分为两步发送: 先发送数据的长度，然后发送数据的真实信息
        write(clientSock,&strLength,sizeof(unsigned long));
